...
SteppingAction::SteppingAction(DetectorConstruction* det, EventAction* evt)
  :  G4UserSteppingAction(), //from CaloGan.src.SteppingAction.cc
     fDetector(det), //Equivalent to  fDetConstruction(detectorConstruction) in CaloGan.SteppingAction
     fEventAct(evt)
{}
...
//SteppingAction::~SteppingAction() //CaloGan destructor
//{}
...
int SteppingAction::WhichXYbin(double xpos, double ypos, int zbin){  //  CaloGan.SteppingAction
  int xbin = -1;
  int ybin = -1;
  int nbins1x = 3;
  int nbins2x = 12;
...
{
  // track informations
  const G4StepPoint* prePoint = aStep->GetPreStepPoint(); // Called point1 in CaloGan
  G4ThreeVector pos = prePoint->GetPosition(); // Called pos1 in CaloGan
  int mybin = WhichXYbin(pos.x(),pos.y(),WhichZBin(pos.z())); // CaloGan.SteppingAction
